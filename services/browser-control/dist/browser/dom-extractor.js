 ...
 */
export async function extractPageState(page, options = {}) {
 ...
 */
export async function extractPageStateLite(page) {
 ...
 */
export async function extractPageStateCompact(page, options = {}) {
    const opts = { ...DEFAULT_STATE_OPTIONS, ...options };
    const [url, title] = await Promise.all([
        page.url(),
        page.title(),
 ...
 */
async function extractFilteredHTML(page, maxLength = 30000) {
 ...
 */
async function extractTextContent(page, maxLength = 3000) {
    return page.evaluate(`
    (function() {
      const maxLen = ${maxLength};
 ...
 */
async function extractInteractiveElements(page, maxElements = 75) {
